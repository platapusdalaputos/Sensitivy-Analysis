...
dLim = np.linspace(9.7e-6,15.7e-6,M)
deltaLim = np.linspace(1e-6,1.3e-6,M)
...
plt.title('TP')
norm = mpl.colors.Normalize(vmin=min(deltaLim), vmax=max(deltaLim))
sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
sm.set_array([])
plt.colorbar(sm, ticks = [min(deltaLim),max(deltaLim)])
plt.show()
