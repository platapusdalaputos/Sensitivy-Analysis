...
''' Sensitivity of the Sa/Vg Ratio '''
...
norm = mpl.colors.Normalize(vmin=min(Sa_VgLim), vmax=max(Sa_VgLim))
sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
sm.set_array([])
plt.colorbar(sm, ticks = [min(Sa_VgLim),max(Sa_VgLim)])
plt.show()
...
#     plt.ylabel('Signal Amplitude a.u.')
#     plt.title('Tissue Phase (TP) Sa/Vg Ratio')
...
#     plt.xlabel('Delay Time (s)')
#     plt.ylabel('Signal Amplitude a.u.')
#     plt.title('RBC Phase (RBC) Sa/Vg Ratio')    
    # plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.15),fancybox=True, shadow=True, ncol=5)
